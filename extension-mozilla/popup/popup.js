...
resetElement.onclick = function () {
	...
	browser.runtime.sendMessage({ event: 'resetSliders', prefs })
}
...
submitElement.onclick = function () {
	...
	if (horizontalpos.value) {
		...
		browser.runtime.sendMessage({ event: 'horizChange', prefs })
	}
	if (verticalpos.value) {
		const prefs = {
			vert: verticalpos.value
		}
		browser.runtime.sendMessage({ event: 'vertChange', prefs })
	}
	if(imagescale.value) {
		const prefs = {
			scale: imagescale.value
		}
		browser.runtime.sendMessage({event: 'scaleChange', prefs})
	}
}
...
browser.storage.local.get(["link", "trans", "ptrans", "horiz", "vert", "scale"], (result) => {
